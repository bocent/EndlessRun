...
public class TrackManager : MonoBehaviour
{
    ...
    protected bool m_IsTutorial = false; //Tutorial is a special run that don't chance section until the tutorial step is "validated" by the TutorialState.
    ...
    private float questionSpawnTime = 10f;
    ...
    {
        SetScore(QuizController.Instance.score);
    ...
    public IEnumerator Begin()
    {
        if (!m_Rerun)
        {
            ...
            player = Instantiate(Resources.Load<Character>("Bundles/Characters/Cat/character"));
            yield return null;
    ...
    public void End()
    {
        ...
        {
            m_PastSegments[i].gameObject.SetActive(false);
...
//"level" in our game are milestone the player have to reach : one every 300m
    ...
    public IEnumerator SpawnNewSegment()
    {
        ...
        //    yield break;
        //}
        TrackSegment newSegment = Instantiate(m_CurrentThemeData.zones[m_CurrentZone].prefabList[segmentUse].GetComponent<TrackSegment>(), _offScreenSpawnPos, Quaternion.identity);//(segmentToUseOp.Result as GameObject).GetComponent<TrackSegment>();
        yield return null;
    ...
    public void SpawnObstacle(TrackSegment segment)
    {
        if (segment.possibleObstacles.Length != 0)
        {
            for (int i = 0; i < segment.obstaclePositions.Length; ++i)
            {
                ...
                Obstacle obstacle = segment.possibleObstacles[Random.Range(0, segment.possibleObstacles.Length)].GetComponent<Obstacle>();
                if (obstacle != null)
                    StartCoroutine(obstacle.Spawn(segment, segment.obstaclePositions[i]));
            }
        }
        ...
        if (!m_IsTutorial)
        {
            ...
            while (currentWorldPos < segment.worldLength)
            {
                ...
                if (laneValid)
                {
                    ...
                    if (elapsedTime >= questionSpawnTime)
                    {
                        ...
                        //if (consumableDatabase.consumbales[(int)ConsumableType.INVINCIBILITY].canBeSpawned)
                        {
                            // Spawn a powerup instead.
                            m_TimeSincePowerup = 0.0f;
                            powerupChance = 0.0f;

                            //AsyncOperationHandle op = Addressables.InstantiateAsync(consumableDatabase.consumbales[(int)ConsumableType.INVINCIBILITY - 1].gameObject.name, pos, rot);
                            //yield return op;
                            //if (op.Result == null || !(op.Result is GameObject))
                            //{
                            //    Debug.LogWarning(string.Format("Unable to load consumable {0}.", consumableDatabase.consumbales[(int)ConsumableType.INVINCIBILITY - 1].gameObject.name));
                            //    yield break;
                            //}
                            toUse = Instantiate(consumableDatabase.consumbales[(int)ConsumableType.INVINCIBILITY - 1].gameObject, pos, rot); //op.Result as GameObject;
                            toUse.transform.SetParent(segment.transform, true);
                            Debug.LogWarning("spawn : " + toUse.name, toUse);
                            elapsedTime = 0;
                            yield return null;
                        }
                    }
